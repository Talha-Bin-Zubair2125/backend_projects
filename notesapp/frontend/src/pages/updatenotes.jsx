...
function Updatenotes() {
  ...
  useEffect(() => {
    const fetchNote = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/notes/getnote/${id}`, { withCredentials: true });
        setnewTitle(response.data.title);
        setnewContent(response.data.content);
      } catch (error) {
        console.error("Failed to fetch note:", error.response ? error.response.data : error.message);
      }
    };
    fetchNote();
  }, [id]);
  ...
  const updateNote = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:3000/api/notes/updatenote/${id}`, data, { withCredentials: true });
      navigate("/getnotes");
    } catch (error) {
  ...
  return (
    <div className="updatenotes-wrapper">
      ...
      <div className="updatenotes-hero">
        ...
        <h1>Edit your <span>note.</span></h1>
        ...
        <div className="updatenotes-card">
          ...
          <form className="updatenotes-form" onSubmit={updateNote}>
            ...
            <div className="updatenotes-btn-row">
              <button type="submit" className="btn-updatenote-submit">Save Changes</button>
              <button type="button" className="btn-updatenote-back" onClick={() => navigate("/getnotes")}>Back</button>
            </div>
          </form>
